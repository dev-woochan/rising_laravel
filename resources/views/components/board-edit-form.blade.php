<main class="container mx-auto bg-white rounded-lg p-6 mt-8 overflow-hidden shadow">
    <form action="{{route('boards.update', $board)}}" class="write_form"  enctype="multipart/form-data" id="write_form" method="POST" >
    @csrf
    @method('patch')
        <div class="form_wrap">
            <div class="flex flex-wrap mb-1">
                <div class="w-full md:w-1/2 pr-2 mb-1 md:mb-0">
                    ...
                    <input type="text" class="write_textarea border border-gray-500 rounded-md px-4 py-2 w-full" name="title" id="titleInput" value="{{$board->title}}">
                </div>
            ...
            <div class="flex flex-wrap mb-1">
                <div class="w-full md:w-1/3 pl-2 mb-1 md:mb-0">
                    ...
                    <div class="flex pr-5" >
                        <input type="text" class="write_textarea border border-gray-300 rounded-l-md px-4 py-2 w-full" name="stock_name" id="stockInput" list="stocks" autocomplete="off" value="{{$board->stock_name}}">
                ...
                <div class="w-full md:w-1/3 pr-2 mb-1 md:mb-0">
                    ...
                    <input type="number" name="stock_price" id="priceInput"  value="{{$board->stock_price}}"class="border border-gray-300 rounded-md px-4 py-2 w-full">
                </div>
                <div class="w-full md:w-1/2 pl-2">
                    <label for="datepicker" class="block">목표일</label>
                    <input type="text" name="target_date" id="datepicker" class="datepicker border border-gray-300 rounded-md px-4 py-2" value="{{$board->target_date}}" autocomplete="off">
<script src="{{asset("js/datepicker.js")}}"></script>                </div>
            </div>
            <div class="write_button_left mb-4">
                ...
                <input type="hidden" name="rise_select" id="select" value="{{$board->rise_select}}">
            </div>
            <script src="{{asset("/js/selectbutton.js")}}"></script>

            <textarea class="main_text border border-gray-300 rounded-md px-4 py-2 w-full" name="editordata" id="summernote">
            {{$board->content}}
            </textarea>
